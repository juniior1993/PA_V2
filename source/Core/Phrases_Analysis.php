            ...
            while ($continue) {
                ...
                if (!array_key_exists($segment, $this->segmentsAndRepetitions) && !in_array($segment, $this->blackList)) {
                    ...
                    if ($result < $this->minRepetitions) {
                        $continue = false;
                        $numberOfWords = $this->minWords;
                        if($this->haveARepetition) {
                            $this->segmentsAndRepetitions[$lastResult["result"]][$lastResult["segment"]] = $lastResult["numberOfWords"];
                            $this->allText = str_replace($lastResult["segment"], " ", $this->allText);
                            $this->haveARepetition = false;
                        }

                    } else {
                        $this->haveARepetition = true;
                        $lastResult["result"] = $result;
                        $lastResult["segment"] = $segment;
                        $lastResult["numberOfWords"] = $numberOfWords;
